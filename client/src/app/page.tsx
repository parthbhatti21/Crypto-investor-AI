...
const STEPS = [
  { n: "1", icon: "🔌", title: "Connect Wallet", desc: "Click Connect Freighter in the top-right. Approve access in the Freighter extension." },
  { n: "2", icon: "🔮", title: "Make a Prediction", desc: "Pick a crypto asset, choose UP or DOWN, set a target price, stake some XLM, and submit." },
  { n: "3", icon: "🏆", title: "Earn Rewards", desc: "Other users back your prediction. When the deadline passes, correct predictions claim the pool." },
];
            ...
            <div className="mx-auto max-w-6xl px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 mt-0.5">Prediction Market · Stellar Testnet</p>
                </div>
              </div>
          ...
          {!CONTRACT_ADDRESS && (
            <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 mt-4">
              <div className="rounded-xl bg-amber-500/8 border border-amber-500/20 px-4 py-3 text-sm text-amber-300 flex items-center gap-2">
                <span>⚠️</span>
                <span>Contract not deployed — set <code className="bg-amber-500/10 px-1 rounded text-xs font-mono">NEXT_PUBLIC_CONTRACT_ADDRESS</code> in <code className="bg-amber-500/10 px-1 rounded text-xs font-mono">.env.local</code></span>
              </div>
            </div>
          )}
          ...
          <main className="flex-1 mx-auto w-full max-w-6xl px-4 sm:px-6 py-10 space-y-14">
            ...
            <div className="text-center space-y-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-violet-500/20 bg-violet-500/8 px-4 py-1.5">
                ...
                <span className="text-xs font-semibold text-violet-300 uppercase tracking-wider">Live on Stellar Testnet</span>
              </div>
              ...
              <p className="text-zinc-400 max-w-xl mx-auto text-base leading-relaxed">
                Make crypto price predictions on the Stellar blockchain. Back others' predictions, and claim rewards when you're right.
              </p>
              {!address && (
            ...
            {/* ── How it works (shown to everyone, fades after connect) ── */}
            {!address && (
              <div className="space-y-4">
                <h2 className="text-center text-xs font-bold text-zinc-500 uppercase tracking-widest">How it works</h2>
                <div className="grid sm:grid-cols-3 gap-4">
                  {STEPS.map((s) => (
                    <div key={s.n} className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5 space-y-2">
                      <div className="flex items-center gap-3">
            ...
            <div className="grid lg:grid-cols-5 gap-8 items-start">
              ...
              <div className="lg:col-span-3 space-y-6">
                ...
                {/* Wallet actions (only when connected) */}
                {address && (
                  <>
